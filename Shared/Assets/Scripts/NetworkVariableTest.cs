    ...
    void Update()
    {
        ...
        if (IsServer)
        {
            ...
            if (t_now - last_t > 0.5f)
            {
                ...
                if (OutputStream)
                {
                    OutputStream.text = "Server set its var to: " + ServerNetworkVariable.Value + ", has client var at: " + ClientNetworkVariable.Value;
                }
            }
        }
        else if (IsClient)
        {
            ...
            if (t_now - last_t > 0.5f)
            {
                if (OutputStream)
                {
                    OutputStream.text = "Client set its var to: " + ClientNetworkVariable.Value + ", has server var at: " + ServerNetworkVariable.Value;
                }
            }
        }
    }
